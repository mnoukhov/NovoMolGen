...
class WandbCallback(TrainerCallback):
    ...
    def __init__(self, model, entity: str, project: str, name: str, config: dict, tags: list, mode: str= 'online',
                 run_evaluation: bool = False, resume: str = None, log_weight_grad_norm: bool = False):
        ...
        if state.is_world_process_zero:
            ...
            if self._wandb.run is None:
                self._wandb.init(entity=self.entity, project=self.project, name=self.name,
                                 config=self.config, tags=self.tags, mode=self.mode, resume=self.resume)
    ...
    def on_log(self, args, state, control, model=None, logs=None, **kwargs):
        ...
        if state.is_world_process_zero:
            ...
            if hasattr(state, "evaluation_task_results") and getattr(state, "evaluation_task_results") is not None:
                logs.update(state.evaluation_task_results)
                state.evaluation_task_results.clear()
